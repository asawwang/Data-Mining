...
num_tree = 8
new_row_size = len(rows)/num_tree
new_row_size = int(new_row_size)
forest = []
for i in range(0, num_tree):
    starting = i * new_row_size
    # print(type(starting))
    ending = max((i+1)*new_row_size, len(rows))
    # print(type(ending))
    new_row = rows[starting:ending]
    my_tree = build_tree(new_row)
...
for test_row in test_rows:
    ...
    forest_decisions_dic = {}
    for i in range(0, num_tree):
        curr_tree = forest[i]
        curr_tree_decision = majority_voting(classify(test_row, curr_tree))
        if curr_tree_decision not in forest_decisions_dic:
            forest_decisions_dic[curr_tree_decision] = 0
        forest_decisions_dic[curr_tree_decision] += 1
    # print(forest_decisions_dic)
    classified_class = majority_voting(forest_decisions_dic)
